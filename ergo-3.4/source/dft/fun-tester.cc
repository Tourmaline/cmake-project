...
void test_var(real comp, real refer, const char* fun, const char* drv, 
              int* counter)
{
      if(std::fabs(comp-refer)>2e-7+5e-5*(std::fabs(comp)+std::fabs(refer))) { 
        if(*counter<95)
            printf("%s %s: fin.diff: %12g found: %12g, diff=%g\n", 
                   fun, drv, (double)refer, (double)comp, (double)std::fabs(comp-refer));
	++*counter;
...
{ 
	...
	for(j=1; j<=40; j+=GRID_STEP) {
		...
		/* TEST df00001 */
		if(std::fabs(gracos)<1e-5) continue;
		dgra = gracos*1e-7;
